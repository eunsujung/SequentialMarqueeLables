...
@property (nonatomic, assign) BOOL isFirstMarqueeStart, isFirstMarqueeEnd;
...
//noatomic -> atomic
@property (copy, atomic) void (^MLAnimationCompletionBlock)(BOOL finished);
...
@synthesize tclButtonId, delegate, m_strButtonID, subLabel;
...
- (void)returnLabelToOriginImmediately {
    ...
    self.MLAnimationCompletionBlock = nil;
}
...
- (void)scrollAwayWithInterval:(NSTimeInterval)interval delayAmount:(NSTimeInterval)delayAmount shouldReturn:(BOOL)shouldReturn {
    ...
    self.MLAnimationCompletionBlock = ^(BOOL finished) {
...
- (void)scrollContinuousWithInterval:(NSTimeInterval)interval
    ...
    self.MLAnimationCompletionBlock = ^(BOOL finished) {
        if (!finished || !weakSelf) {
            ...
            if (! _isFirstMarqueeStart) {
                NSLog(@"ck-firstWatingStart");
                //[self.subLabel setHidden:YES];

                [NSTimer scheduledTimerWithTimeInterval:delayAmount
                                                 target:self
                                               selector:@selector(timeoutHandler:)
                                               userInfo:@"firstWatingEnd"
                                                repeats:NO];
                _isFirstMarqueeStart = YES;
            }
            NSLog(@"ck---1");
            return;
        }
        ...
        if (weakSelf.window && ![weakSelf.subLabel.layer animationForKey:@"position"]) {
            ...
            if (weakSelf.labelShouldScroll && !weakSelf.tapToScroll && !weakSelf.holdScrolling) {
                NSLog(@"ck-ScrollEnd");
                [self.delegate didEndScroll:nil];
...
-(void)timeoutHandler:(NSTimer*) timer
{
    ...
    if ([info isEqualToString:@"firstWatingEnd"]) {
        if (! _isFirstMarqueeEnd) {
            NSLog(@"ck-firstWatingEnd");
            //[self.subLabel setHidden:NO];
            _isFirstMarqueeEnd = YES;
            
            [self.delegate didStartScroll:nil];
    ...
    if ([info isEqualToString:@"scrollMarqueeStart"]) {
        //[self.subLabel setHidden:NO];
    }
    [timer invalidate];
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if ([anim isMemberOfClass:[GradientSetupAnimation class]]) {
        ...
        if (self.MLAnimationCompletionBlock) {
            self.MLAnimationCompletionBlock(flag);
        }
    }
}
